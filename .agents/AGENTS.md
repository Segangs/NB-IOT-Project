...
- **깃 커밋 메시지 한글 및 간결 작성**:
  - 작업 완료 시마다 자동으로 Git commit/push를 진행하지 말고, 사용자가 명시적으로 커밋/동기화/푸시를 요청할 때까지 보류하십시오.
  - 사용자가 Git 커밋을 요청한 경우 커밋 메시지는 반드시 **한글(Korean)**로 작성하십시오. (영어 메시지 지양)
...
- **README / project_history 작성 형식**:
  ...
  - `README.md`는 작업 건별 날짜 제목을 반복하지 않고, 날짜별 섹션 1개 안에 해당 날짜의 여러 작업 내용을 순차 누적하십시오.
