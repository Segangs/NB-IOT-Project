...
begin
    if exists (
        ...
        where high_notification_count > 1
    ) then
        raise exception
            'repeat alert data exists; explicit data decision required';
    end if;
end;
$rollback_gate$;
...
alter table public.temperature_alert_outbox
    ...
    drop constraint temperature_alert_outbox_notification_event_check,
    ...
    drop column notification_ordinal;
...
alter table public.temperature_alert_state
    ...
    drop column last_high_notification_at,
    drop column high_notification_count;
