...
do $command_wire_compact_payload_down$
declare
...
begin
    ...
    if pg_catalog.strpos(
        ...
        ''
            ...
            raise exception using
                errcode = '55000',
                message = 'command response rollback source drift';
        end if;
        ...
        if v_rewritten is not distinct from v_definition then
            raise exception using
                errcode = '55000',
                message = 'command response rollback rewrite failed';
        end if;
        execute v_rewritten;
    end if;
    ...
    if pg_catalog.strpos(
        ...
        ''
            ...
            raise exception using
                errcode = '55000',
                message = 'command ACK rollback source drift';
        end if;
        v_rewritten := pg_catalog.replace(
            v_definition,
            $new$v_payload := pg_catalog.replace(
        ...
        if v_rewritten is not distinct from v_definition then
            raise exception using
                errcode = '55000',
                message = 'command ACK rollback rewrite failed';
        end if;
        execute v_rewritten;
    end if;
...
$command_wire_compact_payload_down$;
