...
- local PostgreSQL rehearsal: PostgreSQL 17.10 sanitized fixture에서 precheck→up→verify→down→legacy verify PASS
...
rollback은 기존 command 원본을 삭제하지 않지만, 신규 companion 상태 이력 삭제를 일으킨다. 여기에는 delivery attempt, lease, accepted/final ACK, device clock metadata가 포함된다. 따라서 live activation 뒤 rollback 전에는 companion table export와 사용자 별도 승인이 필요하다.

## 후속 gate

- 완료: local PostgreSQL 17.10 sanitized fixture의 precheck→up→verify→down→legacy verify rehearsal
